...
//		PrintWriter fileWriter=null; //出力ファイルライター
		...
		do {
			...
			for (int i = 1; i < N - 1; i++) {
				for (int j = 1; j < N - 1; j++) {
					...
					phi[i][j] = 0.25 * (rho[i][j] * delta * delta / e0 + phi[i + 1][j] + phi[i - 1][j] + phi[i][j + 1]
							+ phi[i][j - 1]);
					// Poisson方程式のphiを計算
		...
		// 電場の出力
		file = new File("Outputfile/electric.avd");
		try {
			fileWriter = new PrintWriter(new BufferedWriter(new FileWriter(file)));
			for (int i = 1; i < N-1; i++) {
				for (int j = 1; j < N-1; j++) {
					...
					fileWriter.printf("%e %e %e %e %e\n", delta*i, delta*j,  Math.sqrt(Ex*Ex+Ey*Ey), Ex, Ey);
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
